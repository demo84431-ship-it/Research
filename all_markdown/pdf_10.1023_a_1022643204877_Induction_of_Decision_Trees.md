...
but its greater complexity makes it suspect as an 'explanation' of the training set.
2
4. ID3
...
jects having outcome O. This is represented graphically by the tree form of Figure
i
4. If each subset C in this figure could be replaced by a decision tree for C, the result
i i
would be a decision tree for all of C. Moreover, so long as two or more C's are non-
i
empty, each C is smaller than C. In the worst case, this divide-and-conquer strategy
i
...
If attribute A with values {A , A , ... A ) is used for the root of the decision tree,
1 2 v
it will partition C into (C , C , ... C } where C contains those objects in C that
1 2 V i
have value A of A. Let C contain p objects of class P and n of class N. The expected
i i i i

90 J.R. QUINLAN
information required for the subtree for C is I(p, n). The expected information re-
i i i
...
to C. The information gained by branching on A is therefore
i
A good rule of thumb would seem to be to choose that attribute to branch on which
gains the most information. ID3 examines all candidate attributes and chooses A to
3
maximize gain(A), forms the tree as above, and then uses the same process recursively
to form decision trees for the residual subsets C , C , ... C .
1 2 v
...
A special case arises if C contains no objects with some particular value A of A,
j
giving an empty C. ID3 labels such a leaf as 'null' so that it fails to classify any object
j
arriving at that leaf. A better solution would generalize from the set C from which
C came, and assign this leaf the more frequent class in C.
j
...
untested attribute A must be determined. This gain in turn depends on the values p
i

92 J.R. QUINLAN
and n for each value A of A, so every object in C must be examined to determine
i i
...
both classes, and let A be an attribute with random values that produces subsets {C ,
1
C , ... C }. Unless the proportion of class P objects in each of the C is exactly the
2 v i
...
subsets {C , C , ... C } of C, where C contains p and n objects of class P and N,
1 2 v i i i
respectively. If the value of A is irrelevant to the class of an object in C, the expected
value p' i of pi should be
If n'i is the corresponding expected value of n, the statistic
i
...
value chosen at random from among the values that could have appeared. Table 2
5
...
Noise Single All Class
level attribute attributes information
5% 1.3% 11.9% 2.6%
10% 2.5% 18.9% 5.5%
15% 3.3% 24.6% 8.3%
20% 4.6% 27.8% 9.9%
...
ability that the object has value A of A by examining the distribution of values of
i
A in C as a function of their class. Suppose that the object in question belongs to
class P. The probability that the object has value A for attribute A can be expressed
i
as
where the calculation of p and p is restricted to those members of C whose value of
i
...
Suppose A is an attribute with values {A , A } and let C be a collection of objects
1 2
such that
giving a value of 1 bit for E(A). Now let A' be an identical attribute except that one
of the objects with value A of A has an unknown value of A'. A' has the values
1
(A' , A' , A' = unknown), so the corresponding values might be
1 2 3
...
with values {A , A , ... A }. For some collection C of objects, let the numbers of
1 2 v
objects with value A of A be p and n and let p and n denote the numbers of ob-
i i i, u u
...
Thus the gain is assessed as if the true value of p were given by
i
where
and similarly for n. (This expression has the property that unknown values can only
i
...
before forming decision trees for the subsets {C}.
i
...
The correct procedure would take the branch corresponding to the real value A but,
i
...
passed a token with some value T. In the situation above, each branch of A is then
i
...
Analysis supports this finding. Let A be an attribute with values A , A , ... A
1 2 v
...
have only two outcomes. If we have an attribute A as before with v values A , A ,
1 2
... A , the decision tree no longer has a branch for each possible value. Instead, a
v
...
was of the form 'attribute A has value A', with two branches corresponding to true
i
...
the distinct values of A that occur in C are sorted to give the sequence V , V , ....
1 2
Vk. Each pair of values V, V+ suggests a possible threshold
i i 1
...
subsets are removed, there are still 2V-1-1 different ways of specifying the
...
A have values A , A , ... A and let the numbers of objects with value A of attribute
1 2 v i
A be p and n respectively. Enquiring about the value of attribute A itself gives rise
i i
...
values, however, will fragment the training set C into very small subsets {C} and
i
